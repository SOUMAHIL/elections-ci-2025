...
# ── CACHE MÉMOIRE SÉMANTIQUE ──────────────────────────────────────────────────
@st.cache_resource
def get_semantic_memory(api_key: str) -> SemanticMemory:
    """
    Crée la mémoire sémantique UNE SEULE FOIS pour toute la session.
    @st.cache_resource = même objet réutilisé à chaque rerun Streamlit.
    L'index FAISS est chargé depuis disque au démarrage, pas à chaque question.
    """
    return SemanticMemory(api_key)
...
""", unsafe_allow_html=True)
    ...
    # ── STATS MÉMOIRE SÉMANTIQUE ─────────────────────────────────────
    # FIX bug 3 : on passe par session_state pour afficher les stats
    # car semantic_memory n'est pas encore initialisée à ce stade du code
    nb_souvenirs = st.session_state.get("nb_souvenirs_memoire", 0)
    if nb_souvenirs > 0:
        st.caption(f"🧠 {nb_souvenirs} souvenir(s) en mémoire")
...
def donnees_graphiables(df, question: str) -> bool:
    q = question.lower()
    num_cols = [c for c in df.columns if df[c].dtype in ["int64","float64","int32","float32"]]
    if not num_cols or len(df) < 2:
        return False
    mots_auto = ["top", "classement", "siège", "siege", "taux", "participation",
                 "comparaison", "répartition", "repartition", "combien de sièges",
                 "remporté", "gagné par parti"]
    return any(m in q for m in mots_auto)
...
def generer_graphique(df, question: str):
    try:
        ...
        y_col = (next((c for c in num_cols if "voix" in c.lower()), None)
                 or next((c for c in num_cols if "siege" in c.lower() or "siège" in c.lower()), None)
        ...
        def get_couleur(label, idx=0):
            l = str(label).upper()
            for parti, col in COULEURS_PARTIS.items():
                if parti in l:
                    return col
            return PALETTE_DEFAUT[idx % len(PALETTE_DEFAUT)]

        def clean_label(s, maxlen=30):
            s = str(s).strip()
        ...
        for i, row in df.iterrows():
            ...
            c = get_couleur(parti_val, i) if parti_val and parti_val != "nan" else get_couleur(nom_val, i)
            couleurs.append(c)
        ...
        SOURCE = "Source : CEI — Élections législatives CI, 27 déc. 2025"

        est_siege = any(w in q for w in ["siège", "siege", "nb_sieges", "remporté", "sièges remportés"]) \
                    or "siege" in y_col.lower()
        ...
        if est_siege and len(df) >= 2:
            fig = go.Figure(go.Pie(
                ...
                hovertemplate="<b>%{label}</b><br>%{value} sièges (%{percent})<extra></extra>",
                hole=0.35,
            ))
            fig.update_layout(
                **layout_base,
                ...
                legend=dict(orientation="h", y=-0.15, x=0.5, xanchor="center",
                            font=dict(size=11)),
                annotations=[
                    dict(text=SOURCE, xref="paper", yref="paper",
                         x=0.5, y=-0.25, showarrow=False,
                         font=dict(size=10, color="#aaa"), align="center"),
                ],
            )
            return fig

        if est_taux and len(df) <= 15:
            hover_fmt = [f"<b>{l}</b><br>Taux : {v:.2f}%" for l, v in zip(labels, values)]
            text_vals = [f"{v:.1f}%" for v in values]
            fig = go.Figure(go.Bar(
                x=list(range(len(labels))),
                y=list(values),
                marker=dict(color=couleurs,
                            line=dict(color="white", width=1.5), opacity=0.92),
                text=text_vals,
            ...
            fig.update_layout(
                **layout_base,
                ...
                xaxis=dict(
                    ...
                    showline=False, color="#444",
                ),
                yaxis=dict(
                    title="Taux (%)",
                    gridcolor="#e5e7eb",
                    showline=False, color="#666",
                ...
                annotations=[
                    dict(text=SOURCE, xref="paper", yref="paper",
                         ...
                         font=dict(size=10, color="#aaa"), align="left"),
                ],
            )
            return fig
        ...
        fig = go.Figure(go.Bar(
            ...
            y=labels_inv,
            orientation="h",
            marker=dict(color=couleurs_inv,
                        line=dict(color="white", width=1.5), opacity=0.92),
        ...
        fig.update_layout(
            **layout_base,
            height=max(380, 80 + len(labels) * 42),
            margin=dict(l=20, r=20, t=70, b=90),
            xaxis=dict(
                title="Voix obtenues",
                gridcolor="#e5e7eb",
...
# Initialiser la mémoire sémantique (cached — chargée une seule fois)
semantic_memory  = get_semantic_memory(api_key)
...
# Chercher un souvenir similaire AVANT d'appeler Mistral
contexte_memoire = semantic_memory.search(current_query)
...
with st.chat_message("assistant"):
    ...
    if intent == "BLOCKED":
        ...
        with st.spinner("🔍 Analyse en cours..."):
            try:
                ...
                # ── Construction du prompt avec mémoire sémantique ────
                prompt_sql = f"{SQL_SYSTEM_PROMPT}\n\n"
                if contexte_memoire:
                    prompt_sql += f"{contexte_memoire}\n\n"
                if historique_ctx:
                ...
                if validate_sql(sql_clean):
                    ...
                    if sql_result.success and not sql_result.data.empty:
                        ...
                        afficher_graphique = (
                            veut_graphique or
                            donnees_graphiables(sql_result.data, clean_question)
                        )

                        if afficher_graphique and len(sql_result.data) >= 2:
    ...
    # ── SAUVEGARDER DANS L'HISTORIQUE + MÉMOIRE SÉMANTIQUE ───────────────────
    # FIX bug 1+2 : indentation correcte, save() DANS le if response
    if response:
        ...
        # Sauvegarder uniquement les réponses SQL et RAG (pas erreurs, pas salutations)
        if intent in ("SQL", "RAG") and len(response) > 30:
            semantic_memory.save(current_query, response)
            # Mettre à jour le compteur pour la sidebar
            st.session_state["nb_souvenirs_memoire"] = semantic_memory.stats()["nb_souvenirs"]
