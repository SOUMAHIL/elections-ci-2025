"""
prompts.py — Version 6
============================
Fixes v6 :
  - Règle 2bis : exception pour noms déjà complets avec virgule
    (MARCORY, COMMUNE) → ILIKE '%MARCORY%' standard
  - Ajout exemples MARCORY, YOPOUGON, KORHOGO avec virgule
"""
...
2bis. RÈGLE ANTI-SURMATCHING — noms courts (≤ 5 lettres) SANS apostrophe :
   ...
   ❌ WHERE c.nom_circ ILIKE '%MAN%'   ← INTERDIT (matche MANKONO, AMANVI...)

   EXCEPTION 1 — nom déjà complet avec virgule dans la question :
   Si la question contient un nom suivi d'une virgule comme
   "MARCORY, COMMUNE", "YOPOUGON, COMMUNE", "KORHOGO, VILLE",
   "COCODY, COMMUNE", "ABOBO, COMMUNE", "BOUAKE, VILLE" etc.
   → le nom est déjà précis, utiliser ILIKE '%NOM%' standard.
   ✅ WHERE c.nom_circ ILIKE '%MARCORY%'    (question : "MARCORY, COMMUNE")
   ✅ WHERE c.nom_circ ILIKE '%YOPOUGON%'   (question : "YOPOUGON, COMMUNE")
   ✅ WHERE c.nom_circ ILIKE '%KORHOGO%'    (question : "KORHOGO, VILLE")
   ✅ WHERE c.nom_circ ILIKE '%COCODY%'     (question : "COCODY, COMMUNE")
   ❌ WHERE (c.nom_circ ILIKE 'MARCORY,%' OR ...) ← INTERDIT si nom déjà complet

   EXCEPTION 2 — noms avec apostrophe (M', N') :
   Ces noms apparaissent au MILIEU des noms de circs composés, jamais au début.
   → TOUJOURS utiliser ILIKE '%partie_après_apostrophe%'
   ...
   1. Nom déjà complet avec virgule dans la question → ILIKE '%NOM%' standard
   2. Nom avec apostrophe (M', N')                  → ILIKE '%partie%'
   3. Nom court ≤ 5 lettres sans apostrophe          → ILIKE 'MOT,%' OR ILIKE 'MOT %'
   4. Nom long 6+ lettres sans apostrophe            → ILIKE '%MOT%' standard

   Exemples corrects :
   "MARCORY, COMMUNE"  → WHERE c.nom_circ ILIKE '%MARCORY%'
   "YOPOUGON, COMMUNE" → WHERE c.nom_circ ILIKE '%YOPOUGON%'
   "KORHOGO, VILLE"    → WHERE c.nom_circ ILIKE '%KORHOGO%'
   "COCODY, COMMUNE"   → WHERE c.nom_circ ILIKE '%COCODY%'
   "BOUAKE, VILLE"     → WHERE c.nom_circ ILIKE '%BOUAKE%'
   ...
   "Tabou"             → WHERE c.nom_circ ILIKE '%Tabou%'
   "Bouaké"            → WHERE c.nom_circ ILIKE '%Bouak%'
...
Q: Qui a gagné à MARCORY, COMMUNE ?
SQL: SELECT r.nom_candidat, r.nom_parti, r.voix_obtenues, r.pourcentage, c.nom_circ
     ...
     WHERE c.nom_circ ILIKE '%MARCORY%' AND r.est_elu = 'OUI'
     ORDER BY r.voix_obtenues DESC LIMIT 10;

Q: Qui a gagné à YOPOUGON, COMMUNE ?
SQL: SELECT r.nom_candidat, r.nom_parti, r.voix_obtenues, r.pourcentage, c.nom_circ
     FROM resultats r JOIN circonscriptions c ON r.code_circ = c.code_circ
     WHERE c.nom_circ ILIKE '%YOPOUGON%' AND r.est_elu = 'OUI'
     ORDER BY r.voix_obtenues DESC LIMIT 10;

Q: Qui a gagné à COCODY, COMMUNE dans le District d'Abidjan ?
SQL: SELECT r.nom_candidat, r.nom_parti, r.voix_obtenues, r.pourcentage, c.nom_circ
     FROM resultats r JOIN circonscriptions c ON r.code_circ = c.code_circ
     WHERE c.nom_circ ILIKE '%COCODY%' AND r.est_elu = 'OUI'
     ORDER BY r.voix_obtenues DESC LIMIT 10;
